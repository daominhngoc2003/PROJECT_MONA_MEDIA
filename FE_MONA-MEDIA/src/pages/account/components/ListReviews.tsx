...
import { getBillByIdUserReviews } from "../../../api/Bill";
...
const ListReviews = () => {
    ...
    useEffect(() => {
        (async () => {
            try {
                const { data } = await getBillByIdUserReviews(req as any);

                if (data.success) {
                    dispatch({ type: "bill/getBIllByUserReview", payload: data.bills })
                }
                else {
    ...
    const onHandleRepurchase = async (bill: any) => {
        if (bill) {
            bill.products.map(async (item: any) => {
                ...
                try {
                    const responsive: any = await AddToCart(dataCart);
